...
class syntaxError:
	...
	def __repr__(self):
		arrow_p = ('   '+' '*self.pos) + '^'
		error_string = arrow_p+'\n'+self.fn+' Col:'+str(self.pos)+' -> ! '+self.type+': Syntax Error - !'
		return error_string
...
class Lexer():
		...
		while self.text[self.pos] in numbers + '.' + '-':
			...
			if currChar == '.':
				...
				if len(n) > 0:
					...
					negative += 1
					if negative > 1:
						return error(self.fn, self.pos, "Multiple Negative Signs")
	...
	def makeVariable(self, starting_pos):
		var = ''

		while self.text[self.pos] in letters + numbers +  '_':
			currChar = self.text[self.pos]
			...
			if self.pos < len(self.text):
				...
				return error(self.fn, self.pos, "Incorrect Variable Syntax")

		return Token(NAME, var, starting_pos, len(var))
	...
	def Lex(self):
		...
		while self.pos < len(self.text):
			...
			if currChar in ' \t':
				...
				if word.value in reqkMathInterpreter.global_variables:
					num = reqkMathInterpreter.global_variables[word.value]
					if type(num) == int:
						tokens.append([Token(INT, int(num), self.TK_IDX, len(str(num)))])
						self.TK_IDX += 1
					elif type(num) == float:
						tokens.append([Token(FLOAT, float(num), self.TK_IDX, len(str(num)))])
						self.TK_IDX += 1
				...
				elif word.value.lower() == 'asn':  # variable lexing section
					tokens.append([Token('ASN', None, self.TK_IDX, 3)])  # variable token
					self.TK_IDX += 1
					self.pos += 1
					...
					while self.text[self.pos] == ' ':
						if self.pos < len(self.text)-1:
							...
							return error(self.fn, 4, "Incorrect Syntax")

					if self.text[self.pos] in numbers:
						return error(self.fn, self.pos, "Digits at start of variable")

					var_token = self.makeVariable(self.TK_IDX)
					...
					while self.text[self.pos] != ';' and self.text[self.pos] == ' ':
						if self.pos < len(self.text)-1:
							...
							return error(self.fn, 4 + len(var_token.value), "Missing ';'")

					if self.text[self.pos] != ';':
						return error(self.fn, 4 + len(var_token.value), "Missing ';'")

					tokens.append([Token(SEMI_COLON, ';', self.TK_IDX, 1)])
					self.TK_IDX += 1

					while self.text[self.pos] not in numbers + '-':
						if self.pos == len(self.text)-1:
							tokens.append([Token(INT, 0, self.TK_IDX, 1)])
							self.TK_IDX += 1
							return tokens
...
def main(fn, text, showtokens=False, showparse=False, dev_mode=False, show_stages=False):
	...
	if dev_mode == False:
		try:
			...
			if type(current_ops) != list: return current_ops
...
def help():
	print("\n Commands:")
	print("  ':tk' - Toggles token visibility")
	print("  ':pt' - Toggles parsed token visibility")
